...
suffix is optional""").format(options
  ...
  ####################################################################
  def _vdoPackerThreadsOptionParser(self, noDefault = False):
    """
    Arguments:
      noDefault (boolean) - if True, no default is mentioned in the help text
    """
    defaultHelp = ("" if noDefault else
                   _("The default is {0}.").format(Defaults.packerThreads))

    parser = argparse.ArgumentParser(add_help = False)
    parser.add_argument("--vdoPackerThreads",
                        type = self.__optionCheck(
                                  Defaults.checkThreadCount1_100),
                        metavar = "<threadCount>",
                        help = _("""
      Specifies the number of threads that pack the compressed block of data.
      The value must be at least {min} and less than or equal to {max}. 
      {defaultHelp}
                                 """)
